...
if ($arrlist.Length -eq 4) {
    ./Tentacle.exe register-with --instance "Tentacle" --server $SERVER --apiKey $APIKEY --publicHostName=$ipv4 --space $SPACE --role $arrlist[0] --role $arrlist[1] --role $arrlist[2] --role $arrlist[3] --environment $ENVIRONMENT --comms-style TentaclePassive --force --console
}
