...
elseif ($ENVIRONMENT -eq "07.PRD EU"){
    ...
    ./Tentacle.exe register-with --instance "Tentacle" --server $SERVER --apiKey $APIKEY --name "$NAME-MX" --publicHostName=$ipv4 --space $SPACE --role "MX" --environment $ENVIRONMENT --comms-style TentaclePassive --force --console
    ./Tentacle.exe register-with --instance "Tentacle" --server $SERVER --apiKey $APIKEY --name "$NAME-BR" --publicHostName=$ipv4 --space $SPACE --role "BR" --environment $ENVIRONMENT --comms-style TentaclePassive --force --console
    ./Tentacle.exe register-with --instance "Tentacle" --server $SERVER --apiKey $APIKEY --name "$NAME-EU" --publicHostName=$ipv4 --space $SPACE --role "PT" --role $array[3] --environment $ENVIRONMENT --comms-style TentaclePassive --force --console
}
